...
async function sendTelegram(req, res, next) {
  try {
    ...
    const response = await broadcastService.sendTelegram(message, { route_id, trip_id, status }, req.userId);
...
async function sendViber(req, res, next) {
  try {
    const { message, route_id, trip_id, status } = req.body;
    const response = await broadcastService.sendViber(message, { route_id, trip_id, status }, req.userId);
